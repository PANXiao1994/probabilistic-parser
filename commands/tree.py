...
class Tree(object):
	...
	def dostr(self):
		return "(%s %s)" % (self.label, self.word) if self.is_terminal() \
			   else "(%s %s)" % (self.label, " ".join(map(repr, self.subs)))

	def tosent(self): # Turn the tree to a sentence
		return  self.word if self.is_terminal() \
			   else " ".join(map(str, self.subs))

	def __str__(self):
		if True or self._str is None:
			self._str = self.tosent()
		return self._str
	...
	def parse(line, trunc=False):
		...
		if tree.label != "":
			# create another node
			tree = Tree(label="", span=tree.span, subs=[tree])
	...
	def deBinarize(self):
		...
		if self.subs is not None:
			if len(self.subs) > 0 and len(self.subs[-1].label) > 0:
